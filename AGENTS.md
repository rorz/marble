...
13. **Create files larger than 350 lines of code.** If a file you are authoring or extending is heading past 350 LoC, stop and split it. Extract cohesive subsystems into sibling modules; lift generic primitives into `packages/lib`, `packages/ui`, `packages/contracts`, or whichever existing library home matches the kind of code (see "Repository Convention Discipline"). Before splitting, search for an existing module that already owns the responsibility — duplication is worse than a long file, and reuse is the first move. The 350 LoC ceiling applies to authored source (`.ts`, `.tsx`, `.css`, `.sql`, `.json` config), not to generated artifacts (`*.generated.ts`, lockfiles, squashed migrations, seed dumps). If you genuinely believe a file must exceed 350 LoC (e.g. a single dense state machine that loses meaning when fragmented), ask before crossing the line — do not silently let it grow.
14. **Use `function` declarations.** All authored TypeScript must use arrow functions (`const x = () => {}`) instead of `function` statements. This is enforced by `harness/enforce-arrow-function.ts` and will fail `bun check`. Exemptions: generator functions (`function*`), TypeScript function-overload groups, and test callback parameters. To opt out of a specific violation, use: `// harness-ignore: enforce-arrow-function -- <one-line justification>`.
15. **Forward-reference top-level bindings.** All top-level value bindings (`const`, `let`, `function`, `class`, `enum`) must be declared above their first use within the file. Reading top-to-bottom should never require jumping ahead to find the definition of something already in play. This is enforced by `harness/no-forward-reference.ts` and will fail `bun check`. Type-only references and recursive self-references are exempt. To opt out of an intentional cycle (e.g. mutual recursion that cannot be linearized), use: `// harness-ignore: no-forward-reference -- <one-line justification>`.
16. **Make environment variables required by default.** In `src/env.ts` files, `.optional()` is forbidden unless absence is a real supported mode, not merely a local-development convenience. This is enforced by `harness/no-optional-env.ts` and will fail `bun check`. To opt out of a truly conditional variable, use: `// harness-ignore: no-optional-env -- <one-line justification>`.
17. **Do not swallow caught errors.** A `catch` block must bind the thrown value and use it to log, toast/report, wrap with `cause`, or rethrow. Anonymous catches, empty catches, and unused caught errors are forbidden. This is enforced by `harness/no-swallowed-errors.ts` and will fail `bun check`. Intentional parser/fallback cases must opt out with: `// harness-ignore: no-swallowed-errors -- <one-line justification>`.
