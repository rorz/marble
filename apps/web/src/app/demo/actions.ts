...
export async function loadTableData(tableId: string) {
	...
	if (columnIds.length === 0) {
		return {
			...
			rows: rowsRes.data,
			cells: [] as CellRow[],
			dependencies: [] as Database["public"]["Tables"]["column_dependency"]["Row"][],
		};
	}

	const [cellsRes, depsRes] = await Promise.all([
		supabase.from("cell").select("*").in("column_id", columnIds),
		supabase.from("column_dependency").select("*").in("source_column_id", columnIds),
	]);
...
export async function deleteColumn(columnId: string) {
	...
	await supabase
		...
		.delete()
		.or(
			`source_column_id.eq.${columnId},target_column_id.eq.${columnId}`,
		);
...
export async function createRow(tableId: string) {
	...
	const { data: row, error } = await supabase
		.from("row")
		.insert({ table_id: tableId, index: nextIndex })
		.select()
		.single();
	if (error) throw error;
	...
	if (columns && columns.length > 0) {
		const { data: newCells } = await supabase
			.from("cell")
			.insert(columns.map((c) => ({ column_id: c.id, row_id: row.id })))
			.select();
		return { row, cells: newCells ?? [] };
	}

	return { row, cells: [] as NonNullable<typeof row>[] };
}
...
export async function executeRun(input: {
	...
	cellId: string;
	cellValue?: string;
}): Promise<{ success: boolean; output: unknown; runId: string }> {
	const supabase = db();
