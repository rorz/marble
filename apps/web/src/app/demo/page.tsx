...
export default function DemoPage() {
  ...
  const colDefs = useMemo<ColDef[]>(() => {
    return [
      ...
      ...sortedColumns.map((col) => {
        ...
        return {
          headerName: isInput ? `Col ${col.index} (input)` : `Col ${col.index}`,
  ...
  const runProgramChain = useCallback(
    async (params: {
      ...
      const chainColumnIds = getExecutionOrder(params.startColumnId, depGraph);
      ...
      for (const colId of chainColumnIds) {
        ...
        for (const row of targetRows) {
          ...
          const cellValue =
            isStart && isUserInputColumn(col) ? params.cellValue : undefined;
  ...
  const handleRunAll = useCallback(async () => {
    ...
    for (const col of sorted) {
      ...
      if (isUserInputColumn(col)) {
        log.push(`keep  Col ${col.index} — user input, values unchanged`);
        continue;
      }
            ...
            <button
              ...
              onClick={() => {
                ...
                setEditingProgram({
                  ...
                  output_schema: {} as unknown as Program["output_schema"],
                  first_party: false,
                } as Program);
              }}
            ...
            {programs.map((p) => (
              <button
                ...
                className={`w-full text-left px-4 py-2.5 border-b border-neutral-800/30 hover:bg-neutral-800/50 transition-colors ${
                  editingProgram?.id === p.id ? "bg-neutral-800/60" : ""
                }`}
              >
                ...
                <div className="text-[10px] text-neutral-600 mt-0.5 flex gap-2">
                  ...
                  <span className="ml-auto">{shortId(p.id)}</span>
                </div>
              </button>
            ))}
            {programs.length === 0 && (
          ...
          {editingProgram && (
            <ProgramEditor
              ...
              onSave={async (data) => {
                ...
                if (isNewProgram) {
                  const created = await actions.createProgram({
                    code: data.code,
                    runtime: data.runtime,
                    external_instance_type: data.instanceType,
                  ...
                  const updated = await actions.updateProgram(
                    editingProgram.id,
                    {
                      code: data.code,
                      runtime: data.runtime,
              ...
              onDelete={
                isNewProgram
                  ? undefined
                  : async () => {
                      await actions.deleteProgram(editingProgram.id);
                      setPrograms((prev) =>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <label className="block">
            ...
            <select
              value={instanceType}
              onChange={(e) =>
                setInstanceType(e.target.value as typeof instanceType)
              }
              className={inputClass}
            >
              {[
...
function AddColumnModal({
  ...
  const allMapped = Object.entries(mappings).every(
    ([, m]) =>
      m.source === "cell_value" || (m.source === "column" && m.column_id),
  );
          ...
          {Object.entries(variables).length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {Object.entries(variables).map(([key, varDef]) => {
                  const isCellValue = !!varDef.$marble__use_cell_value;
                    ...
                    >
                      ...
                      {isCellValue ? (
                        <div className="text-xs text-neutral-500">
                          Sourced from the cell's editable value
                        </div>
                      ) : (
